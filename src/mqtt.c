...
)
{
    ...
    /* ship it! don't care for errors, they will be handled in
     * modbus module.
     */
...
)
{
    ...
    {
        /* message frame must be exactly 22 bytes long, no funny
         * bussiness
         */
...
)
{
    ...
    for (i = 0; i != m2md_array_size(g_m2md_mqtt_subs); ++i)
    {
                    ...
                    g_m2md_mqtt_subs[i].topic) > sizeof(topic))
        {
            ...
            el_print(ELE, "cannot subscribe to %s topic to long, made this: %s",
        ...
        {
            el_perror(ELE, "mosquitto_subscribe(%s)", topic);
            continue;
        }
...
)
{
    ...
    {
        el_print(ELC, "mosquitto_reconnect()");
...
)
{
    ...
    for (;;)
    {
        ...
        if (g_m2md_run == 0)
        {
            /* someone lost his nerve and send us SIGTERM,
             * enough is enough
             */
...
)
{
    ...
    {
        el_print(ELE, "topic turned to be too large: %d, made this: %s",
                toplen, top);
        return -1;
    }
    ...
    {
        el_perror(ELE, "mosquitto_publish(%s)", top);
        return -1;
    }
...
)
{
    ...
    {
        el_print(ELE, "m2md_mqtt_create_ack_msg() failed");
        return -1;
    }
